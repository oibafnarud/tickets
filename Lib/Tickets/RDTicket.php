...
use FacturaScripts\Plugins\fsRepublicaDominicana\Model\NCFVentas;
use FacturaScripts\Plugins\fsRepublicaDominicana\Extension\Model\FacturaCliente;
...
function getDescripcionTipoComprobante($numero) {
    $descripciones = array(
    ...
    // Devuelve la descripción correspondiente al número
...
class Normal
{
    ...
    public static function print($doc, TicketPrinter $printer, User $user, Agente $agent = null,): bool
    {
        ...
        $ticket->body .= static::getBigText($company->nombre, $printer->linelen)
            ...
            . $i18n->trans('cifnif') . ': ' . $doc->cifnif . "\n"
            . $i18n->trans('NCF') . ': ' . $doc->numeroncf . "\n"
            . $i18n->trans('tipocomprobante') . ': ' . getDescripcionTipoComprobante($doc->tipocomprobante) . "\n"
            . $i18n->trans('due-date') . ': ' . $doc->ncffechavencimiento . "\n";
